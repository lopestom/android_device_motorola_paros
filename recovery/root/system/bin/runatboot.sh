...
mount /vendor_dlkm
mount /vendor
modprobe -d /vendor_dlkm/lib/modules /vendor_dlkm/lib/modules/focaltech_0flash_v2_mmi.ko
modprobe -d /vendor_dlkm/lib/modules /vendor_dlkm/lib/modules/ili9882_mmi.ko
sleep 1
if [ $(cat /sys/class/touchscreen/primary/productinfo) == "ft8725" ]
then
echo 1 > /sys/class/touchscreen/primary/forcereflash
echo focaltech_ts_fw_.bin > /sys/class/touchscreen/primary/doreflash
...
if [ "$is_fastboot" != "running" ]; then
    mount /vendor_dlkm
        ...
        insmod /vendor_dlkm/lib/modules/focaltech_0flash_v2_mmi.ko
